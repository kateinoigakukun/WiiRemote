...
typedef NS_ENUM(NSInteger, WiiButtonType)  {
	WiiRemoteAButton,
	WiiRemoteBButton,
	WiiRemoteOneButton,
	WiiRemoteTwoButton,
...
static unsigned char mii_data_buf[WIIMOTE_MII_DATA_BYTES_PER_SLOT + 16];
static unsigned short mii_data_offset;

typedef NS_ENUM(NSInteger, WiiExpansionPortType) {
	WiiExpUknown,
	WiiExpNotAttached,
	WiiNunchuk,
...
typedef NS_ENUM(NSInteger, WiiAccelerationSensorType) {
	WiiRemoteAccelerationSensor,
	WiiNunchukAccelerationSensor
};

typedef NS_ENUM(NSInteger, WiiPressureSensorType) {
	WiiBalanceBoardPressureSensor
};


typedef NS_ENUM(NSInteger, WiiJoyStickType) {
